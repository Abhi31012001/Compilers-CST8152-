...
pBuffer stringLiteralTable;	/* this buffer implements String Literal Table */
...
extern int initScanner(Buffer* sc_buf);
extern Token processToken(void);
...
int main(int argc, char** argv) {
	...
	pBuffer sourceBuffer;	/* pointer to input (source) buffer */
...
/* __DATE__, __TIME__, __LINE__, __FILE__, __STDC__ are predefined preprocessor macros*/
	...
	sourceBuffer = bufferCreate(INIT_CAPACITY, INC_FACTOR, 'm');
	...
	loadSize = bufferLoad(fileHandler, sourceBuffer);
	...
	if (bufferSetEnd(sourceBuffer, '\0')) {/* '\0' */ /* EOF */
		displayScanner(sourceBuffer);
	}

	/* create string Literal Table */
	stringLiteralTable = bufferCreate(INIT_CAPACITY, INC_FACTOR, 'a');
	...
	if (bufferGetAddCPosition(stringLiteralTable)) bufferPrint(stringLiteralTable, 1);
	bufferFree(sourceBuffer);
...
void displayScanner(Buffer* ptrBuffer) {
	...
	printf("The capacity of the buffer is:  %d\n", bufferGetCapacity(ptrBuffer));
	printf("The current size of the buffer is:  %d\n", bufferGetAddCPosition(ptrBuffer));
	printf("\nPrinting buffer contents:\n\n");
	bufferRewind(ptrBuffer);
	bufferPrint(ptrBuffer, 1);
...
void printToken(Token t) {
	...
	case STR_T:
		...
		printf("%s\n", bufferGetString(stringLiteralTable,
			(short)t.attribute.get_int));
		break;
	case SCC_OP_T:
		printf("SCC_OP_T\n");
		break;
